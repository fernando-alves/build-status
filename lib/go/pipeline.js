module.exports = function Pipeline(name, history) {
  ...
  (function parseHistory() {
    const pipelineResult = stages => {
      const failed = stage => stage.result.toLowerCase() === 'failed';
      return stages.some(failed) ? 'failed' : 'passed';
    };

    const latestScheuledJob = stages => {
      const jobs = stages.reduce((jobs, stage) => jobs.concat(stage.jobs), []);

      const scheduledTimestamps = jobs.map(job => job.scheduled_date);
